...
Working sketch:  `HelloWorld/HelloWorld.ino` (display verified, SW reset path works)
...
- [ ] Create `Clock/` directory with empty `Clock.ino`, `pins.h`, `version.h` (set to `0.1.0`).
- [ ] Create `Clock/build_scripts/` and copy `compile.sh`, `monitor.sh` from `HelloWorld/build_scripts/` (already targets the right FQBN). Update `config.env` with `SKETCH_NAME="Clock.ino"`.
- [ ] Create `Clock/upload_usb.sh` — copy from reference and adapt port glob.

**Acceptance:** `cd Clock && ./build_scripts/compile.sh` builds an empty sketch successfully.

### T2 — Library setup  *(small, ~5 min)*
- [ ] Confirm installed: `arduino-cli lib list | grep -E "GxEPD2|GFX|BusIO|ArduinoJson|WebSockets|ArduinoOTA"`.
- [ ] Install missing: `arduino-cli lib install "ArduinoJson"`, `arduino-cli lib install "WebSockets"`.
  (GxEPD2, Adafruit GFX, Adafruit BusIO already verified per session notes.)
- [ ] Note: `ArduinoOTA`, `WiFi`, `WebServer`, `Preferences`, `esp_task_wdt`, `esp_sntp` ship with the ESP32 core.

### T3 — WiFi + NTP  *(medium, ~20 min)*
- [ ] Port `DebugLog.{h,cpp}` from reference unchanged.
- [ ] Port `loadConfig()` / `saveConfig()` / `setDefaultConfig()` / `setupWiFi()` from reference.
- [ ] Port `setupNTP()` with the SNTP callback pattern (don't use the broken `getLocalTime() == true` shortcut — see comments in reference).
- [ ] On boot: load config → connect WiFi → sync NTP → log real time. (No display yet — verify via serial.)
- [ ] If WiFi fails: log error, continue.
...
- [ ] Port `WebSerialHandler.{h,cpp}` from reference (depends on `WebSocketsServer` from T2).
- [ ] Port `setupWebServer()` + handlers (`/config`, `/status`) from reference. Drop battery fields.
- [ ] Port `ArduinoOTA.setHostname("WeAct-Clock"); ArduinoOTA.begin();`.
- [ ] **Replace EXT0 maintenance trigger:** on every boot, run an 8-second window where `ArduinoOTA.handle() + server.handleClient() + WebSerial.loop()` are pumped before sleeping/proceeding.
- [ ] During this phase, just `delay()` after the 8 s window instead of sleeping (since deep sleep comes in T6).
...
- [ ] Copy `upload.sh` from reference to `Clock/build_scripts/`. Update `ESPOTA` path: `~/Library/Arduino15/packages/esp32/hardware/esp32/3.3.5/tools/espota.py`.
- [ ] Verify path exists; if not, find it and update.
- [ ] Test: `./build_scripts/compile.sh && ./build_scripts/upload.sh <device-ip>`.
- [ ] Confirm `version.h` auto-increments on success.
...
- [ ] Copy `HebrewClock.{h,cpp}` from reference unchanged.
- [ ] Copy `fonts/NotoSansHebrew22pt.h` and `fonts/NotoSansHebrew24pt.h` from reference.
- [ ] Write `pins.h` with the 7 confirmed pin mappings (copy from `HelloWorld.ino:11-17`).
- [ ] In `Clock.ino`: include GxEPD2_BW for `GxEPD2_420_GDEY042T81`, render two-line Hebrew time using `HebrewClock::getTimeStrings()` + `drawHebrewText()` from real NTP time (T3).
- [ ] First boot: `display.init(115200, true)` + full refresh. Minute updates: `display.display(true)` partial refresh.
...
- [ ] Port `goToSleep()` from reference (sub-second precision via `gettimeofday`, drift compensation, 50 ms margin).
- [ ] **Drop EXT0 wakeup setup** (no buttons, and C6 needs different API anyway).
- [ ] Skip `esp_sleep_pd_config(ESP_PD_DOMAIN_RTC_FAST_MEM, ...)` — verify it's still valid on C6 (it is, per ESP-IDF docs, but no harm if dropped since we don't use fast memory).
- [ ] On timer wake: `display.init(115200, false)` (controller keeps its frame), update only if minute changed, partial-refresh (`display.display(true)`).
- [ ] On power-on / OTA-window wake: full init + full refresh.
...
- [ ] Verify text fits at 22pt and 24pt. Try larger fonts (`gen_gfx_font.py` if needed) since the panel is 400×300 vs reference's 296×128.
- [ ] Footer strip (~40 px tall): IP left, version center, NTP status right, all in 9pt mono.
- [ ] During the 8 s OTA window after a power-on boot, show a small corner indicator (e.g., a dot or "OTA").

**Acceptance:** Layout looks balanced; nothing clipped on left/right/top.

### T9 — Documentation  *(small, ~10 min)*
- [ ] Write `Clock/README.md`: build, USB upload, OTA upload, web endpoints, where config lives.
- [ ] Add a top-level `README.md` in `WeAct_Eink_Clock/` linking to HelloWorld + Clock + Documentation.
